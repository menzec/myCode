...
def download_tile(tileX, tileY, zoom, base_url='http://www.google.cn/maps/vt', map_type='s'):
    map_paras = {
        "lyrs": map_type,
        'gl': 'en',
        'x': str(tileX),
    ...
    try:
        r = requests.get(base_url, params=map_paras, headers=headers)
        r.raise_for_status()
        img = r.content
        r.close()
...
def download_single_image(tile_coor, zoom, size, base_url=None):
    image_temp_name = 'temp'
    image_list = []
    ...
    for y in range(int(tile_coor[1] - y_step / 2), int(tile_coor[1] + y_step / 2) + 1):
        for x in range(int(tile_coor[0] - x_step / 2), int(tile_coor[0] + x_step / 2) + 1):
            if base_url:
                image_list.append(download_tile(x, y, zoom, base_url))
            else:
                image_list.append(download_tile(x, y, zoom))
    # 拼接下载的瓦片
...
def batch_download_image(filename, img_save_dir, zoom, size, base_urls=None):
    with open(filename, 'r') as fp:
        ...
        logfile.write('file %s has %d coordinates.\n' %
                      (os.path.basename(filename), num))
        for data in coor_info:
            try:
                ...
                img = download_single_image(
                    tile_coor, zoom, (x_length, y_length), base_url=cur_url)
                img.save(image_name)
                logfile.write('%sDownload %d/%d pictures,succeed!' %
                              (str(datetime.datetime.now()), i, num))
                if i % 200:
                    print(datetime.datetime.now(),
                          'Download %d/%d pictures,succeed!' % (i, num))
                ...
                print(err, '\nMaxRetryError/MaxRetryError Error! Rest 600s')
                print(
                    err, '\nMaxRetryError/MaxRetryError Error! Rest 600s', file=logfile)
                logfile.flush()
                change_url(base_urls)
                file_size = 0
                time.sleep(600)
...
def main():
    ...
    download_file_dir = r"D:\data\tongji\downloadfile"
    save_dir = r'D:\data\tongji\image\L_17-304'
    ...
    for file in download_files:
        ...
        batch_download_image(
            cur_download_file, cur_save_dir, zoom, size, base_urls)
    print('Finish')
