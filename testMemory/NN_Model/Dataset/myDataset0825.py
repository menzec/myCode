...
class UCMerced():
    ...
    def __loader(self, image_name):
        return Image.open('%s/%s' % (self.root, image_name))

    def getdata(self):
        if not os.path.exists(self.root + '/test.txt'):
            divideTrTe(self.root, rate=0.8, file_extension=['.tif'])
        transform_train = transforms.Compose([
            transforms.RandomCrop(256, padding=32),  # 先四周填充0，在吧图像随机裁剪成256*256
        ...
        train_data = MyDataset(
            self.root + '/train.txt', self.__loader, transform=transform_train)
        test_data = MyDataset(
            self.root + '/test.txt', self.__loader, transform=transform_test)
        train_loader = DataLoader(
            dataset=train_data, batch_size=self.batch_size, shuffle=True)
        test_loader = DataLoader(
            dataset=test_data, batch_size=self.batch_size,)
...
##                                 (0.2023, 0.1994, 0.2010)),
        ...
        testloader = torch.utils.data.DataLoader(
            testset, batch_size=int(self.batch_size/2), shuffle=False, num_workers=2)
...
class myCifar10_train_normalize():
    # 准备数据集并预处理
...
class myCifar10_horizonflip_RandomCrop():
    # 准备数据集并预处理
...
class myCifar10_horizonflip_Normalize():
    # 准备数据集并预处理
...
class myCifar10_RandomCrop_Normalize():
    # 准备数据集并预处理
