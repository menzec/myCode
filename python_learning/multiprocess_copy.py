<<<<<<< HEAD:python_learning/multiprocess_copy.py
...
class Search_files(object):
    ...
    def multi_thread_search(self, queue, thread_num=8, filenames='', *args, **kwargs):
        '''多进程查找
        thread_num 进程数
        *args， **kwargs 为search 函数的参数对 ---建议使用kwargs将参数以字典形式传递过来'''
        try:
            ...
            while index < len(filenames):
                ...
                # 线程执行结果为一个对象，get()函数可以获得 该线程执行的函数的 函数返回值
                find_result = thr_pool.apply_async(
                    func=self.search, kwds=deepcopy(kwargs))#args=(filenames[index],),
                result.append([filenames[index], find_result])
                index += 1
        except Exception as ex:
            print("Error:%s" % (ex))
        ...
        *args， **kwargs 为search 函数的参数对 ---建议使用kwargs将参数以字典形式传递过来'''
        try:
            ...
            while index < len(filenames):
                ...
                # 线程执行结果为一个对象，get()函数可以获得 该线程执行的函数的 函数返回值
                result = pro_pool.apply_async(
                    func=self.multi_thread_search, args=(queue, thread_num), kwds=deepcopy(kwargs))
                # 如果不休眠则会 报错----原因未知
                time.sleep(0.00001)
                index += thread_num
        except Exception as ex:
            ...
            while os.getppid() and i < all_num:
                if queue.empty():
                    ...
                    if len(result[1]):
                        copy_files(result)
                        print('%d-%d-' %
                              (i, all_num), result, file=log)
                        sys.stdout.write('%d-%d-%s finish!\n' %
                                         (i, all_num, result[0]))
                    else:
                        print('%d-%d-%s-not found' %
                              (i, all_num, result[0]), file=log)
                        sys.stdout.write('%d-%d not found\n' %
                                         (i, all_num), result)
                    log.flush()
                    sys.stdout.flush()
...
if __name__ == "__main__":
    main()
>>>>>>> 45097d95013786426194b0da606ae036651ab08b:NN_Model/multiprocess_copy.py
